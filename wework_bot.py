...
def chat():
    ...
    try:
        ...
        reply = bot.generate_reply(message)
        
        return jsonify({
            'status': 'success',
            'message': message,
            'reply': reply,
            'api_used': 'ark' if bot.ark_api_key else 'local'
        })
...
if __name__ == '__main__':
    ...
    print(f"\n=== 企业微信群机器人启动 ===")
    print(f"服务端口: {port}")
    print(f"Webhook URL: {bot.webhook_url or '未配置'}")
    print(f"ARK API: {'已配置' if bot.ark_api_key else '未配置'}")
    print(f"访问地址: http://localhost:{port}")
    print(f"Webhook地址: http://localhost:{port}/webhook")
    print("=" * 40)
